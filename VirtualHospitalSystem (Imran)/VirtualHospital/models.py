...
class Patients(db.Model, UserMixin):
    ...
    symptom = db.Column(db.String(length=500), nullable=True, default='')
    medicine = db.Column(db.String(length=500), nullable=False, default='')
    dose = db.Column(db.String(length=2), nullable=False, default="0")
    appointed_doctor = db.Column(db.String(length=20), nullable=True, default="")
    owner_id = db.Column(db.Integer(), db.ForeignKey('doctor.id'))
    owner_feedback = db.Column(db.String(length=500), nullable=True, default='')
...
class Doctor(db.Model, UserMixin):
    ...
    def appoint(self, patient):
        patient.owner_id = self.id
        patient.appointed_doctor = self.username
        patient.patient_budget -= self.priceOfAppointment
        db.session.commit()
